...
function snapshot(project) {
  ...
  const logs = existsSync(logsDir) ? readdirSync(logsDir).sort().slice(-8).flatMap((name) => { const path = join(logsDir, name); const stat = lstatSync(path); return stat.isFile() && !stat.isSymbolicLink() ? [{ name, text: tailText(path, 12_000) }] : []; }) : [];
