...
async function download(url, path, fetchImpl) {
  ...
  if (parsed.protocol !== "https:" || parsed.username || parsed.password || ["localhost", "127.0.0.1", "::1"].includes(parsed.hostname)) throw new Error("Refused unsafe media download URL.");
...
async function persistResult(payload, output, kind, fetchImpl) {
  ...
  else if (url) await download(url, mediaPath, fetchImpl);
  else {
...
export async function generateMedia(kind, options = {}) {
  ...
  return { kind, model, ...(await persistResult(payload, output, kind, fetchImpl)) };
}
